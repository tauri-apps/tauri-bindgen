...
const invoke = window.__TAURI_INVOKE__;export interface LudicrousSpeed {
...
export async function functionWithDashes(): Promise<[]> {
  const result = await invoke<[]>("plugin:3d46778dded1f0fb|function-with-dashes",);
  return result
}
export async function functionWithNoWeirdCharacters(): Promise<[]> {
  const result = await invoke<[]>("plugin:3d46778dded1f0fb|function-with-no-weird-characters",);
  return result
}
