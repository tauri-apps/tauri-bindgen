...
const invoke = window.__TAURI_INVOKE__;export async function float32Param(x: number): Promise<[]> {
  const result = await invoke<[]>("plugin:979575fda4ffb8b9|float32-param",{x: x});
  return result
}
export async function float64Param(x: number): Promise<[]> {
  const result = await invoke<[]>("plugin:979575fda4ffb8b9|float64-param",{x: x});
  return result
}
