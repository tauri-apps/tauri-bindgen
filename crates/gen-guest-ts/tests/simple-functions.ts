...
const invoke = window.__TAURI_INVOKE__;export async function f1(): Promise<[]> {
...
export async function f6(a: number, b: number, c: number): Promise<[number, number, number]> {
  const result = await invoke<[number, number, number]>("plugin:d52f0e93c1bb4daa|f6",{a: a, b: b, c: c});
  return result
}
