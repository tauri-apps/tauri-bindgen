...
let float32Param = (x: float): Promise.t<unit> => {
  invoke(~cmd="plugin:b2ded0ef|float32_param", ~payload={"x": x})
}
let float64Param = (x: float): Promise.t<unit> => {
  invoke(~cmd="plugin:b2ded0ef|float64_param", ~payload={"x": x})
}
let float32Result = (): Promise.t<float> => {
  invoke(~cmd="plugin:b2ded0ef|float32_result")
}
let float64Result = (): Promise.t<float> => {
  invoke(~cmd="plugin:b2ded0ef|float64_result")
}
